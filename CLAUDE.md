...
- **Scheduled prints are the other tool that commands the printer on a timer, and the
  ...
  - **Skip, never guess, and always say why.** Firing re-checks its facts fresh, at
    ...
    printing": leveling, homing and a self-check all count as busy), whether the file
    is still there (via a live `1044` for its own folder, never the ambient
    `StateStore.files` cache another browser's folder click could have overwritten),
    and, when the schedule carries a filament mapping, whether each chosen spool
