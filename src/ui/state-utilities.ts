/**
 * State classes, as utilities: the table. DOM-free, so the build can read it too
 * (`scripts/fill-design-tokens.ts`); `state-classes.ts` applies it.
 *
...
export const STATE_UTILITIES: Record<string, Record<string, StateDelta>> = {
  active: {
    ...
    'file-source-tab': SEGMENT_ACTIVE,
