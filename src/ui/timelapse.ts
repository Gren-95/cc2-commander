...
export function renderTimelapse(state: PrinterState): void {
  ...
  for (const video of videos) {
    ...
    const actionBtn = isExported
      ? `<button class="timelapse-play-btn inline-flex items-center justify-center whitespace-nowrap rounded-full border border-line bg-surface px-3 py-1 text-[11px] font-medium text-fg-soft cursor-pointer transition-colors hover:bg-hover pointer-coarse:min-h-9 pointer-coarse:px-3.5" data-url="${escapeAttr(videoUrl)}">${icon('play')} Play</button>`
      : `<button class="timelapse-export-btn inline-flex items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border border-line bg-surface px-3 py-1.5 text-xs font-medium text-fg cursor-pointer transition-colors hover:bg-hover hover:border-fg-muted disabled:opacity-50 disabled:cursor-not-allowed" data-url="${escapeAttr(videoUrl || name)}">${icon('exportFile')} Export</button>`;
